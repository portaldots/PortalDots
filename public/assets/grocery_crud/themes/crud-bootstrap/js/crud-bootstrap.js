$(function(){
	$('.quickSearchButton').click(function(){
		$(this).closest('.crud-bootstrap').find('.quickSearchBox').slideToggle('normal');
	});

	$('.ptogtitle').click(function(){
		if ($(this).hasClass('vsble')) {
			$(this).removeClass('vsble');
			$(this).closest('.crud-bootstrap').find('.main-table-box').slideDown("slow");
		} else {
			$(this).addClass('vsble');
			$(this).closest('.crud-bootstrap').find('.main-table-box').slideUp("slow");
		}
	});

    var create_export_url = function ($this_form) {
        var $crudBootstrapContainer = $this_form.closest('.crud-bootstrap'),
            exportQuery = '';

        $.each($crudBootstrapContainer.find('.filtering_form').serializeArray(), function(i, field) {
        	if (field.value !== '') {
        ...
        $crudBootstrapContainer.find('.export-anchor').attr('href', export_url + '?' + exportQuery)
    };
	...
	$('.filtering_form').submit(function(){
		var crud_page =  parseInt($(this).closest('.crud-bootstrap').find('.crud_page').val(), 10);
		var last_page = parseInt($(this).closest('.crud-bootstrap').find('.last-page-number').html(), 10);

		if (crud_page > last_page) {
			$(this).closest('.crud-bootstrap').find('.crud_page').val(last_page);
		}
		if (crud_page <= 0) {
			$(this).closest('.crud-bootstrap').find('.crud_page').val('1');
		}
		...
		$(this).ajaxSubmit({
			 ...
			 dataType: 'json',
			 beforeSend: function(){
				 this_form.closest('.crud-bootstrap').find('.ajax_refresh_and_loading').addClass('loading');
			 },
			 complete: function(){
				 this_form.closest('.crud-bootstrap').find('.ajax_refresh_and_loading').removeClass('loading');
			 },
			 success:    function(data){
				this_form.closest('.crud-bootstrap').find('.total_items').html( data.total_results);
				displaying_and_pages(this_form.closest('.crud-bootstrap'));

				this_form.ajaxSubmit({
					 success:    function(data){
						this_form.closest('.crud-bootstrap').find('.ajax_list').html(data);
						call_fancybox();
						add_edit_button_listener();
		...
		if ($('.crud-bootstrap').length == 1) { //disable cookie storing for multiple grids in one page
			...
			createCookie('search_text_'+unique_hash,$(this).closest('.crud-bootstrap').find('.search_text').val(),1);
	...
	$('.crud_search').click(function(){
		$(this).closest('.crud-bootstrap').find('.crud_page').val('1');
		$(this).closest('.crud-bootstrap').find('.filtering_form').trigger('submit');
	});

	//検索フォームで Enter キーが押されたら検索を実行するようにする
	...
	$('.search_clear').click(function(){
		$(this).closest('.crud-bootstrap').find('.crud_page').val('1');
		$(this).closest('.crud-bootstrap').find('.search_text').val('');
		$(this).closest('.crud-bootstrap').find('.filtering_form').trigger('submit');
	});
	...
	$('.prev-button').click(function(){
		if( $(this).closest('.crud-bootstrap').find('.crud_page').val() != "1")
		{
			$(this).closest('.crud-bootstrap').find('.crud_page').val( parseInt($(this).closest('.crud-bootstrap').find('.crud_page').val(),10) - 1 );
			$(this).closest('.crud-bootstrap').find('.crud_page').trigger('change');
		}
	});

	$('.last-button').click(function(){
		$(this).closest('.crud-bootstrap').find('.crud_page').val( $(this).closest('.crud-bootstrap').find('.last-page-number').html());
		$(this).closest('.crud-bootstrap').find('.filtering_form').trigger('submit');
	});

	$('.next-button').click(function(){
		$(this).closest('.crud-bootstrap').find('.crud_page').val( parseInt($(this).closest('.crud-bootstrap').find('.crud_page').val()) + 1 );
		$(this).closest('.crud-bootstrap').find('.crud_page').trigger('change');
	});
	...
	$('.ajax_list').on('click','.field-sorting', function(){
		$(this).closest('.crud-bootstrap').find('.hidden-sorting').val($(this).attr('rel'));

		if ($(this).hasClass('asc')) {
			$(this).closest('.crud-bootstrap').find('.hidden-ordering').val('desc');
		} else {
			$(this).closest('.crud-bootstrap').find('.hidden-ordering').val('asc');
		}
	...
	$('.ajax_list').on('click','.delete-row', function(){
		...
		var this_container = $(this).closest('.crud-bootstrap');
	...
	$('.print-anchor').click(function(){
		...
		var form_input_html = '';
		$.each($(this).closest('.crud-bootstrap').find('.filtering_form').serializeArray(), function(i, field) {
		...
		$(this).closest('.crud-bootstrap').find('.hidden-operations').html(form_on_demand);

		var _this_button = $(this);

		$(this).closest('.crud-bootstrap').find('#print_form').ajaxSubmit({
			beforeSend: function(){
