...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
        ...
        if (!empty($state_info->search)) {
            ...
            if (is_array($state_info->search)) {
                ...
                if (isset($temp_relation[$state_info->search->field])) {
                    ...
                    $this->like($state_info->table_name. '.'. $state_info->search->field, $state_info->search->text);
                }
            } else {
                $columns = $this->get_columns();
                ...
                foreach ($columns as $column) {
                    if (isset($temp_relation[$column->field_name])) {
                        if (is_array($temp_relation[$column->field_name])) {
                            ...
                            $this->or_like($state_info->table_name. '.'. $temp_relation[$column->field_name], $search_text);
                        }
                        ...
                        $this->or_like($state_info->table_name. '.'. $column->field_name, $search_text);
                    }
                }
            }
        }
    ...
    protected function db_insert_validation()
    {
        ...
        if (!empty($unique_fields)) {
            ...
            if (!$form_validation->run()) {
                ...
                $validation_result->error_message = "同じ項目が既に存在します。";
    ...
    // 元は protected だが、 Home_staff でも利用する為、public に
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
    ...
    protected function _print_webpage($data)
    {
		...
		@page{margin: 5mm}
		#print-table{ color: #000; background: #fff; font-family: -apple-system, BlinkMacSystemFont, \"Helvetica Neue\", Arial, \"Hiragino Kaku Gothic ProN\", メイリオ, Meiryo, sans-serif, \"Apple Color Emoji\", \"Segoe UI Emoji\", \"Segoe UI Symbol\"; font-size: 13px; font-feature-settings: \"palt\"; }
		#print-table table tr td, #print-table table tr th{ border: 1px solid black; border-bottom: none; border-right: none; padding: 4px 8px 4px 4px; font-size: 13px; vertical-align: top; }
		#print-table table tr th{line-height: 1}
		#print-table table tr td{white-space: nowrap; line-height: 1.3;}
		#print-table table{ border-bottom: 1px solid black; border-right: 1px solid black}
		#print-table table tr th{text-align: left;background: #ddd; vertical-align: middle; }
		#print-table table tr:nth-child(odd){background: #eee}
		tr, td, th, .item {page-break-inside: avoid}
		</style>";
        $string_to_print .= "<div id='print-table'>";

        $string_to_print .= '<table width="100%" cellpadding="0" cellspacing="0" ><tr>';
        foreach ($data->columns as $column) {
            $string_to_print .= "<th><div class='item'>".$column->display_as."</div></th>";
        }
        $string_to_print .= "</tr>";
        ...
        foreach ($data->list as $num_row => $row) {
            $string_to_print .= "<tr>";
            foreach ($data->columns as $column) {
                $string_to_print .= "<td><div class='item'>".$this->_trim_print_string($row->{$column->field_name})."</div></td>";
            }
            $string_to_print .= "</tr>";
        }
    ...
    protected function _trim_print_string($value)
    {
        ...
        return strip_tags($value, "<br><ruby><rt>");
    }
    ...
    {
        $this->set_css($this->default_css_path.'/jquery_plugins/chosen/bootstrap-chosen.css');
        ...
        switch ($state_code) {
            ...
            case 17: //print
                ...
                if (!empty($data['search_text'])) {
                    $state_info->table_name = strip_tags($data['table_name']);
                    if (empty($data['search_field'])) {
                        $search_text = !empty($data['search_field']) ? strip_tags($data['search_field']) : '';
