...
@section('content')
    {{-- <form method="post" action="{{ empty($document) ? route('staff.documents.new') : route('staff.documents.update', $document) }}">
        ...
        <app-container>
            <list-view>
                <template v-slot:title>配布資料基本情報</template>
                @foreach ([
                    'name' => '配布資料の名前',
                    'name_yomi' => '配布資料の名前(よみ)',
                    'group_name' => '配布資料団体の名前',
                    'group_name_yomi' => '配布資料団体の名前(よみ)'
                    ...
                    <list-view-form-group label-for="{{ $field_name }}">
                        ...
                        <input id="{{ $field_name }}" class="form-control @error($field_name) is-invalid @enderror" type="text"
                            name="{{ $field_name }}" value="{{ old($field_name, empty($document) ? '' : $document->$field_name) }}"
                            required>
                    ...
                    <list-view-form-group>
                        ...
                        @empty ($document)
                            <div class="text-muted">
                                <i class="fa fa-info-document fa-fw" aria-hidden="true"></i>
                                カスタムフォーム回答の内容を編集するには、まず配布資料情報を保存してください
                            </div>
                        @else
                            <a href="{{ route('staff.forms.answers.create', ['form' => $custom_form, 'document' => $document]) }}"
                ...
                <list-view-form-group>
                    ...
                    <template v-slot:description>
                        配布資料をタグで分類できます(例 : <app-badge primary>ステージ配布資料</app-badge>、<app-badge primary>模擬店</app-badge>、<app-badge
            ...
            <list-view>
                <template v-slot:title>配布資料のメンバー</template>
                ...
                <list-view-form-group label-for="members">
                    ...
                    <template
                        v-slot:description>学籍番号を改行して入力することで複数の学園祭係を追加できます。{{ config('portal.users_number_to_submit_document') - 1 }}人を下回っていても構いません。</template>
            ...
            <list-view>
                    ...
                    <div class="form-radio">
                        <label class="form-radio__label">
                            <input class="form-radio__input" type="radio" name="status" id="statusRadios1" value="pending"
                                {{ old('status', isset($document) ? $document->status : null) === null ? 'checked' : '' }}>
                        ...
                        <label class="form-radio__label">
                            <input class="form-radio__input" type="radio" name="status" id="statusRadios2" value="approved"
                                {{ old('status', isset($document) ? $document->status : null) === 'approved' ? 'checked' : '' }}>
                            <strong>受理</strong><br>
                            <span class="text-muted">参加登録を受理します。当該配布資料は申請機能を利用できるようになります</span>
                        </label>
                        <label class="form-radio__label">
                            <input class="form-radio__input" type="radio" name="status" id="statusRadios3" value="rejected"
                                {{ old('status', isset($document) ? $document->status : null) === 'rejected' ? 'checked' : '' }}>
                ...
                <list-view-form-group label-for="status_reason">
                    ...
                    <textarea id="status_reason" class="form-control @error('status_reason') is-invalid @enderror"
                        name="status_reason"
                        rows="5">{{ old('status_reason', empty($document) ? '' : $document->status_reason) }}</textarea>
