    ...
    >
        <template v-slot:toolbar>
            <a
                ...
                href="{{ route('staff.schedules.create') }}"
            >
        ...
        <template v-slot:activities="{ row }">
            <form-with-confirm
                v-bind:action="`{{ route('staff.schedules.destroy', ['schedule' => '%%SCHEDULE%%']) }}`.replace('%%SCHEDULE%%', row['id'])" method="post"
                v-bind:confirm-message="`予定「${row['name']}」を削除しますか？`"
            >
                @method('delete')
                @csrf
