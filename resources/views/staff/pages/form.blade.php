...
@section('content')
    <form method="post" action="{{ empty($page) ? route('staff.pages.store') : route('staff.pages.update', $page) }}">
        ...
        <app-container fluid>
            <layout-row grid-template-columns="1fr 400px">
                <layout-column>
                    <list-view>
                        <list-view-form-group label-for="title">
                            ...
                            <input id="title" class="form-control @error('title') is-invalid @enderror" type="text"
                                name="title" value="{{ old('title', empty($page) ? '' : $page->title) }}" required>
                            @if ($errors->has('title'))
                        ...
                        <list-view-form-group>
                            ...
                            <markdown-editor input-name="body"
                                default-value="{{ old('body', empty($page) ? '' : $page->body) }}">
                            </markdown-editor>
                            @if ($errors->has('body'))
                                <template v-slot:invalid>
                        ...
                        <list-view-form-group>
                            ...
                            <tags-input input-name="documents" placeholder="配布資料を指定"
                                ...
                                v-bind:autocomplete-items="{{ $documents_autocomplete_items }}"
